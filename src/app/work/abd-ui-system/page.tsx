    ...
    <>
      ...
      <main id="main-content" style={{ background: c.bg }}>
        ...
        <div style={{ padding: "20px clamp(24px, 5vw, 80px) 0" }}>
          <Link href="/#work" style={{ fontFamily: font.sans, fontSize: "var(--text-body)", color: c.muted, textDecoration: "none" }}>
            ...
            <dl style={{ margin: 0 }}>
              {[
                ...
                <div key={k} style={{ marginBottom: "14px" }}>
                  <dt style={{ fontFamily: font.sans, fontSize: "var(--text-body)", fontWeight: 500, color: c.ink, marginBottom: "2px" }}>{k}</dt>
                  <dd style={{ fontFamily: font.sans, fontSize: "var(--text-body)", lineHeight: 1.6, color: c.body, margin: 0 }}>{v}</dd>
                </div>
              ))}
            </dl>
          </div>
...
function Figure({ src, alt, caption, id }: { src: string; alt: string; caption: string; id?: string }) {
  return (
    <figure id={id} style={{ margin: "36px 0", scrollMarginTop: "96px" }}>
      ...
      <figcaption style={{ fontFamily: font.sans, fontSize: "var(--text-body)", lineHeight: 1.6, color: c.muted, margin: "12px 0 0", maxWidth: "780px" }}>{caption}</figcaption>
    </figure>
  );
}
...
const bodyText: React.CSSProperties = {
  fontFamily: font.sans, fontSize: "var(--text-body)", lineHeight: 1.6, color: c.body, margin: "0 0 20px", maxWidth: "780px",
};
const subheading: React.CSSProperties = {
  // Dropped from 20px to 18px so the H3 sits clearly below the
  // parent H2 (clamp 26-36px) at every viewport.
  fontFamily: font.sans, fontSize: "var(--text-article)", fontWeight: 500, color: c.ink, margin: "40px 0 16px", letterSpacing: "-0.01em",
};
const listStyle: React.CSSProperties = { margin: "0 0 20px", paddingLeft: "20px", maxWidth: "780px" };
const liStyle: React.CSSProperties = { fontFamily: font.sans, fontSize: "var(--text-body)", lineHeight: 1.6, color: c.body, marginBottom: "10px" };
const code: React.CSSProperties = {
  fontFamily: "ui-monospace, SFMono-Regular, Menlo, monospace", fontSize: "var(--text-body)", background: c.bgSection, border: `1px solid ${c.border}`, padding: "1px 6px", borderRadius: "3px", color: c.ink,
};
