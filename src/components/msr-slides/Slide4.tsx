    ...
    <div className="relative overflow-hidden" style={{ width: 1440, height: 900, background: "#F8F7F7" }}>
      <div className="relative z-10 flex flex-col px-[88px] pt-[40px] pb-14 h-full min-h-[900px]">
        ...
        <div className="flex gap-8 flex-1">
          ...
          <div className="flex-1 flex flex-col">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <span className="font-body font-medium" style={{ color: "#6B6560", fontSize: 15 }}>Blog Format · Rejected</span>
            </div>
            <div className="rounded-xl p-6 flex-1 relative" style={{ background: "#FFFFFF", border: "1px solid #EEEDEC", opacity: 0.7 }}>
              ...
              <div className="relative z-1">
                ...
                <div className="rounded mt-4 flex items-center justify-center" style={{ width: "100%", height: 56, background: "#F8F7F7", border: "1px dashed #D6D3D1" }}>
                  <span className="font-body" style={{ color: "#D6D3D1", fontSize: 12 }}>Featured image</span>
                </div>
                ...
                <div className="rounded mt-4 flex items-center justify-center" style={{ width: "100%", height: 32, background: "#F8F7F7", border: "1px dashed #D6D3D1" }}>
                  <span className="font-body" style={{ color: "#D6D3D1", fontSize: 12 }}>Comparison table (buried)</span>
                </div>
              </div>
            </div>
            <p className="font-body leading-relaxed mt-4" style={{ color: "#6B6560", fontSize: 12 }}>The user has to read to evaluate. Signal is buried in narrative.</p>
          </div>
          ...
          <div className="flex-1 flex flex-col">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <span className="font-body font-semibold" style={{ color: "#13100C", fontSize: 15 }}>E-Commerce Pattern · Chosen</span>
            </div>
            <div className="rounded-xl p-6 flex-1 relative overflow-hidden" style={{ background: "#FFFFFF", border: "2px solid #C4703A", boxShadow: "0 4px 20px rgba(196,112,58,0.1)" }}>
              <div className="flex flex-col gap-3">
                {ecomElements.map((el) => (
                  <div key={el.label}>
                    <span className="font-display block mb-1.5 uppercase tracking-[0.15em]" style={{ color: "#6B6560", fontSize: 12 }}>{el.label}</span>
                    <div className="rounded flex items-center justify-center" style={{ width: "100%", height: el.h, background: el.label.startsWith("Buy") ? "linear-gradient(135deg, rgba(196,112,58,0.12) 0%, rgba(196,112,58,0.04) 100%)" : "#F8F7F7", border: el.label.startsWith("Buy") ? "1px solid rgba(196,112,58,0.25)" : "1px solid #EEEDEC" }}>
                      {el.label === "Hero image" && <span className="font-body" style={{ color: "#534F4B", fontSize: 12 }}>Product hero · full width</span>}
                      {el.label === "Specs grid" && (
                        <div className="grid grid-cols-3 gap-2 w-full px-3">
                          {["Weight","Drop","Price"].map((spec) => (
                            <div key={spec} className="text-center">
                              <span className="font-body block" style={{ color: "#534F4B", fontSize: 12 }}>{spec}</span>
                      ...
                      {el.label === "Rating" && (
                        <div className="flex items-center gap-1.5">
                          {[1,2,3,4].map((s) => <span key={s} style={{ color: "#C4703A", fontSize: 12 }}>★</span>)}
                          <span style={{ color: "#D6D3D1", fontSize: 12 }}>★</span>
                          <span className="font-body ml-1" style={{ color: "#6B6560", fontSize: 12 }}>4.2 / 5</span>
                        </div>
                      )}
                      {el.label === "Pros / Cons" && (
                        <div className="grid grid-cols-2 gap-4 w-full px-3">
                          <div>
                            <span className="font-body block mb-1" style={{ color: "#6B6560", fontSize: 12 }}>Pros</span>
                            {[1,2,3].map((n) => <div key={n} className="flex items-center gap-1.5 mb-1"><span style={{ color: "#C4703A", fontSize: 12 }}>+</span><div className="rounded-sm" style={{ width: `${70+n*5}%`, height: 3, background: "#D6D3D1" }} /></div>)}
                          </div>
                          <div>
                            <span className="font-body block mb-1" style={{ color: "#6B6560", fontSize: 12 }}>Cons</span>
                            {[1,2].map((n) => <div key={n} className="flex items-center gap-1.5 mb-1"><span style={{ color: "#6B6560", fontSize: 12 }}>&minus;</span><div className="rounded-sm" style={{ width: `${60+n*10}%`, height: 3, background: "#EEEDEC" }} /></div>)}
                          </div>
                        </div>
                      )}
                      {el.label.startsWith("Buy") && <span className="font-body font-semibold" style={{ color: "#C4703A", fontSize: 12 }}>Buy at Amazon &rarr;&ensp;&ensp;Buy at REI &rarr;</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="mt-5 rounded-xl px-7 py-5 relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(196,112,58,0.06) 0%, rgba(196,112,58,0.01) 100%)", border: "1px solid rgba(196,112,58,0.2)" }}>
          <p className="font-heading italic leading-relaxed relative z-10" style={{ color: "#C4703A", fontSize: 17 }}>&ldquo;Long-form editorial is right when the user is learning. An e-commerce layout is right when the user is deciding. MSR&rsquo;s review pages are for deciding.&rdquo;</p>
          <p className="font-body leading-relaxed mt-4 relative z-10" style={{ color: "#6B6560", fontSize: 12 }}>This isn&rsquo;t a store. There&rsquo;s no checkout, no inventory. The UX pattern is borrowed from e-commerce because it matches the user&rsquo;s task, not because it serves a transaction.</p>
        </div>
      </div>
    </div>
  );
