  ...
  return (
    <div className="relative overflow-hidden" style={{ width: 1440, height: 900, background: "#F8F7F7" }}>
      <div className="relative z-10 flex flex-col px-[88px] pt-[40px] pb-14 h-full min-h-[900px]">
        ...
        {/* Sub */}
        <p className="font-body leading-relaxed mb-6" style={{ color: "#6B6560", fontSize: 15, maxWidth: 900 }}>
        ...
        <div className="grid grid-cols-3 gap-5 flex-1">
          {artifacts.map((a) => (
            <div key={a.label} className="rounded-lg p-5 flex flex-col" style={{ background: "#FFFFFF", border: "1px solid #EEEDEC" }}>
              <span className="font-display tracking-[0.14em] uppercase mb-2" style={{ fontSize: 12, color: "#C4703A", fontWeight: 500 }}>{a.label}</span>
              <h2 className="font-body font-semibold leading-tight mb-2" style={{ fontSize: 15, color: "#13100C" }}>{a.title}</h2>
              <p className="font-body leading-snug" style={{ fontSize: 12, color: "#6B6560" }}>{a.body}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-5 rounded-lg px-6 py-3 relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(196,112,58,0.06) 0%, rgba(196,112,58,0.01) 100%)", border: "1px solid rgba(196,112,58,0.2)" }}>
          <p className="font-heading italic leading-relaxed text-center" style={{ color: "#C4703A", fontSize: 15 }}>&ldquo;Personalization, re-engagement, and first-party data can be earned without trading the editorial trust the audience came for.&rdquo;</p>
        </div>
