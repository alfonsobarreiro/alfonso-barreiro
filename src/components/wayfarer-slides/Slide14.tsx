  ...
  return (
    <SlideFrame index={14} title="Evaluation Plan">
      ...
      <h2 className="font-display" style={{
        margin: "16px 0 12px",
        fontSize: "var(--text-h1)", fontWeight: 500, lineHeight: 1.15, letterSpacing: "-0.01em",
        color: wf.n900, maxWidth: 1200,
      }}>
        Concept project. No live traffic. Here’s how I’d test it.
      </h2>
      <p className="font-body" style={{
        margin: "0 0 24px", fontSize: "var(--text-body)", color: wf.n500, maxWidth: 1000,
      }}>
      ...
      <div style={{ border: `1px solid ${wf.n200}`, borderRadius: 8, overflow: "hidden" }}>
        ...
        {rows.map((r, i) => (
          <div key={r[0]} style={{
            ...
            <div className="font-body" style={{ fontSize: "var(--text-body)", fontWeight: 500, color: wf.n900 }}>{r[0]}</div>
            <div className="font-body" style={{ fontSize: "var(--text-body)", color: wf.n700 }}>{r[1]}</div>
            <div className="font-body" style={{ fontSize: "var(--text-body)", fontWeight: 500, color: wf.brand500 }}>{r[2]}</div>
            <div className="font-body" style={{ fontSize: "var(--text-small)", color: wf.n500 }}>{r[3]}</div>
          </div>
        ))}
      </div>
    </SlideFrame>
